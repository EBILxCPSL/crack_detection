...
from evaluate_unet import mpa, jaccard, dice
import shutil
...
def create_model(device, type ='vgg16'):
    '''可以用torchsummary輸入照片大小去看model每層的output'''
    if type == 'vgg16':
...
def train(train_loader, model, criterion, validation, args, model_output_dir):
    ...
    # Or
    # Find the lastest weights
    current_model_path = os.path.join(*[model_output_dir, 'model_epoch_current.pt'])
    if os.path.isfile(current_model_path):
        # current_path = ''
        # for path in Path(args.model_dir).glob('*.pt'):
        #     if 'model_epoch_current' in path.stem:
        #         current_path = os.path.join(*[args.model_dir, 'model_epoch_current.pt'])
        # state = torch.load(current_path)
        # model.load_state_dict(state['model'])
        # current_model_path = os.path.join(*[args.model_dir, 'model_epoch_current.pt'])
        ...
        print(f"\nlatest_model_path is {latest_model_path}")
        best_model_path = os.path.join(*[model_output_dir, 'model_best.pt'])
    ...
    # 使全部權重參數都不做gradient計算，只改變最後一層final層的參數(freeze wieght方法)
    '''Freeze the pretrained weight, and train other layers'''
    if args.finetune_final_layer is True and args.model_type!='efficient':
        ...
        # model.dec3.weight.requires_grad = True
        # model.dec3.bias.requires_grad = True
...
#         model.final.bias.requires_grad = True
        ...
        model.center.block[1].conv.weight.requires_grad = True
        model.center.block[1].conv.bias.requires_grad = True
        model.center.block[2].conv.weight.requires_grad = True
        model.center.block[2].conv.bias.requires_grad = True
        
        
    elif args.finetune_final_layer is True and args.model_type=='efficient':
        model.outputs.weight.requires_grad = True
        model.outputs.bias.requires_grad = True
    else:
        for param in model.parameters():
    ...
    for epoch in range(epoch, args.n_epoch + 1):
        ...
        writer.add_image("Prediction", masks_viz[0], epoch)
...
def validate(model, val_loader, criterion, epoch):
    ...
    with torch.no_grad():
        for i, (input, target, path) in enumerate(val_loader):
            ...
            result_miou += [jaccard(output_eval, target_eval)]
...
if __name__ == '__main__':
    # 多GPU時，指定GPU -> CUDA_VISIBLE_DEVICES=0
    # Change parameter
    ...
    # 不調整lr -> adj_lr = n_epoch+1
    parser.add_argument('-adj_lr', default=777, type=int, help='adjust learning rate every X epoch')
    ...
    parser.add_argument('-batch_size',  default=10, type=int,  help='weight decay (default: 1e-4)')
    ...
    parser.add_argument('-model_type', type=str, required=False, default='vgg16', choices=['vgg16', 'resnet101', 'resnet34', 'efficient'])
    ...
    parser.add_argument('-loss', type=str, required=False, default='bce', choices=['bce', 'dice', 'dicebce', 'iou', 'focal'])
    ...
    parser.add_argument('-split', default=0.85, type=float, help='split the dataset into train and validation')    
    # Save
    parser.add_argument('-save_every_epoch', default=10, type=int, help='save the model every X epoch')
    parser.add_argument('-exp', default='1', type=str, help='save the experiment logs')
    ...
    if os.path.isdir(valid_dataset_path):
...
#                                     '''kernel_size為模糊半徑(必須為奇數); sigma=數字(固定) & two tuples為範圍內隨機'''
#                                     transforms.GaussianBlur(kernel_size=21, sigma=(1.0, 10.0)),
...
#                                     transforms.RandomVerticalFlip(p=0.5),
    ...
    if args.sep:
        print("Manually split the dataset")
        ...
        print(f'\ntraining images = {len(img_names)}')
        print(f'valid images = {len(valid_img_names)}')
    else:
        print("Auto split the dataset")
        # 80% training data ; 10% validation
